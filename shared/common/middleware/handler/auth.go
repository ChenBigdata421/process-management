...
import (
	"encoding/json"
	models "jxt-evidence-system/process-management/shared/admin/domain/aggregate"
	...
	"github.com/ChenBigdata421/jxt-core/sdk/pkg/captcha"
...
func Authenticator(c *gin.Context) (interface{}, error) {
	...
	db, err := pkg.GetOrm(c)
	if err != nil {
		log.Error("get db error", zap.Error(err))
	...
	if e == nil {
		...
		msg = "登录失败"
		status = "1"
		log.Warn("login failed!", zap.String("username", loginVals.Username))
	}
	return nil, jwt.ErrFailedAuthentication
}
...
// GetUserOrgID 从JWT token中获取用户的组织ID
func GetUserOrgID(c *gin.Context) int {
	claims := jwt.ExtractClaims(c)
